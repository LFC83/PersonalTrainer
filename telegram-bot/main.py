...
EQUIPAMENTOS_GIM = ["Elástico", "Máquina Remo", "Haltere 25kg max", "Barra olímpica 45kg max", "Kettlebell 12kg", "Bicicleta Spinning", "Banco musculação/Supino"]
DATA_DIR = '/data'
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # CORREÇÃO: Ordenar atividades corretamente
    activities = load_activities()
    formatted_acts = []
    for act in activities:
        f = format_activity(act)
        if f['date']:
            formatted_acts.append(f)
    
    # Ordenar por data (mais recente primeiro) e pegar últimas 3
    formatted_acts.sort(key=lambda x: x['date'], reverse=True)
    recent_activities_raw = formatted_acts[:3]
    
    # Converter de volta para raw para uso posterior
    recent_activities = [f['raw'] for f in recent_activities_raw]
    ...
    if recent_activities_raw:
        msg += f"\n\n🏃 ÚLTIMAS ATIVIDADES:"
        for formatted in recent_activities_raw:
...
async def analyze_activity_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for i, act in enumerate(formatted_acts[:5], 1):
        label = f"{act['date']} - {act['sport']} ({act['duration_min']}min"
        if act['distance_km']:
            label += f", {act['distance_km']}km"
        label += ")"
        keyboard.append([InlineKeyboardButton(label, callback_data=f'analyze_act_{i-1}')])
    ...
    # Guardar atividades formatadas no contexto
    context.user_data['formatted_activities'] = formatted_acts
...
async def analyze_activity_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    formatted_acts = context.user_data.get('formatted_activities', [])
...
async def perform_activity_analysis(query, context, index, cargo=False):
    ...
    # Pegar atividades dos 3 dias anteriores e 2 dias posteriores
    historical_activities = []
    for i, act in enumerate(formatted_acts):
        if abs(i - index) <= 3 and i != index:
            historical_activities.append(f"- {act['date']}: {act['sport']} ({act['duration_min']}min")
            if act['distance_km']:
                historical_activities[-1] += f", {act['distance_km']}km"
            if act['load']:
                historical_activities[-1] += f", Load {act['load']}"
            historical_activities[-1] += ")"
...
Fornece números e métricas concretas sempre que possível."""
    ...
    try:
        ...
        # Enviar resposta em partes se for muito longa
        ...
        logger.info(f"Análise individual executada para atividade {selected['date']}")
...
def main():
    """Inicia o bot"""
    logger.info("=== FitnessJournal-HRV Bot v2.7 ===")
    ...
    app.add_handler(CallbackQueryHandler(analyze_activity_callback, pattern=r'^analyze_act_'))
